<br>
  <img src="https://scontent.fdac142-1.fna.fbcdn.net/v/t39.30808-6/572996064_1605299090846159_7204266110511570313_n.jpg?stp=dst-jpg_p180x540_tt6&_nc_cat=100&ccb=1-7&_nc_sid=127cfc&_nc_eui2=AeHdZ-J09v7QamMHAmQVGtUkNVl-yeG_ko01WX7J4b-SjXE1gkY5vqqTq3HWaPMFQdjT9ZojNaIC0yKHTag3NBt8&_nc_ohc=mGH8dzH8qrsQ7kNvwGHOhGw&_nc_oc=Adn3Dib4zzaSM6fgWW0pmOKfursDwbUWAP0-OfEJjaqFznphMfKrcWCSG5cINkhh4_g&_nc_zt=23&_nc_ht=scontent.fdac142-1.fna&_nc_gid=hABLZnvj9HVyu83SS-Akfw&oh=00_AfgJoKvN0tx6W3sGMYw8QrvtHpoixck_KC1fO0hDexEnsg&oe=690D6505" alt="facebook_photos_private">

<h1></h1>
<span style="font-size: 50px; font-weight: 900;">Hi! I'm Md. Kalim Ullah</span> 
<br>

<span style="font-size: 30px; font-weight: normal;">Mobile App Developer with Flutter</span> 
<br><br><br>

<span style="font-size: 20px; font-weight: 500;"> I'm a Mobile Apps Developer specializing in Flutter, dedicated to building fast, clean, and user-friendly applications for Android and iOS. I focus on delivering elegant design, smooth performance, and a great user experience in every project.</span>

<br><br>

<!-- devider -->

<hr style="height: 2px; border: none; background-color: #ccc; margin: 2px 0;">
<!-- box-shadow: rgb(0, 221, 255) 0px 2px 4px 0px, rgb(70, 138, 255) 0px 2px 16px 0px -->
<br><br><br>

<div>
  <span style="font-size: 30px; font-weight: 900;"> Skills :</span>
  &nbsp;
  <span style="font-size: 24px; font-weight: 500; display: inline;"> HTML / Tailwind CSS / MySQL / Flutter </span>
</div>

<br><br>
<!-- devider -->
<hr style="height: 1px; border: none; background-color: #ccc; margin: 1px 0;">

<br><br>

<div style="font-size: 24px; line-height: 2;">
  <!-- css -->
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">

  <i class="fa-solid fa-briefcase" style="color: #6c757d;"></i>  I'm currently working on Life Insurance Company<br>
  <i class="fa-solid fa-book-open" cstyle="color: #6c757d;"></i> I'm currently currently learning Python Programming<br>
  <i class="fa-solid fa-handshake" style="color: #6c757d;"></i>  I'm looking to Collaborate on Git & GitHub<br>
  <i class="fa-solid fa-circle-question" style="color: #6c757d;"></i>  Ask me about Mobile App Development
</div>
<br>
<!-- devider -->
<hr style="height: 1px; border: none; background-color: #ccc; margin: 1px 0;">

<br><br>

<span style="font-size: 30px; font-weight: 900;"> 🛠️ Tech Stack :</span>

<div style="display:flex; justify-content:center; align-items: center; text-align: center; flex-wrap:wrap;">

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/flutter/flutter-original.svg" width="50"/>
      <span style="margin: 10px;">Flutter</span>
  </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dart/dart-original.svg" width="50" />
      <span style="margin: 10px;">Dart</span>
   </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="50" />
      <span style="margin: 10px;">HTML5</span>
  </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="50" />
      <span style="margin: 10px;">Tailwind CSS</span>
  </div>
  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="50" />
      <span style="margin: 10px;">MySQL</span>
  </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="50" />
      <span style="margin: 10px;">GitHub</span>
  </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="50" />
    <span style="margin: 10px;">Python</span>
  </div>
</div>
<br>
<!-- devider -->
<hr style="height: 1px; border: none; background-color: #ccc; margin: 1px 0;">

<br><br>

<span style="font-size: 30px; font-weight: 900;"> 🌐 Connect With Me :</span>

<div style="display:flex; justify-content:center; align-items: center; text-align: center; flex-wrap:wrap;">
  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
        <img src="https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/github.svg" width="50" />
      <span style="margin: 10px;">GitHub</span>
  </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
          <img src="https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/facebook.svg" width="50" />
      <span style="margin: 10px;">facebook</span>
   </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
      <img src="https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/adobeillustrator.svg" width="50" />
      <span style="margin: 10px;">Adobe Illustrator</span>
  </div>

  <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; margin: 30px;">
     <img src="https://cdn.jsdelivr.net/npm/simple-icons@3.0.1/icons/adobephotoshop.svg" width="50" />
      <span style="margin: 10px;">Adobe Photoshop</span>
  </div>

</div>

<br>
<!-- devider -->
<hr style="height: 1px; border: none; background-color: #ccc; margin: 1px 0;">

<br><br>

<span style="font-size: 30px; font-weight: 900;"> 🏅 GitHub Badges :</span>
<div style="display:flex; justify-content:center; flex-wrap:wrap;">
  <span style="display:flex; flex-direction:column; align-items:center;  margin:10px;">
    <img src="https://raw.githubusercontent.com/acervenky/animated-github-badges/master/assets/acbadge.gif" width="50" />
  </span>
  <span style="display:flex; flex-direction:column; align-items:center; margin:10px;">
    <img src="https://raw.githubusercontent.com/acervenky/animated-github-badges/master/assets/devbadge.gif" width="50" />
  </span>
  <span style="display:flex; flex-direction:column; align-items:center; margin:10px;">
    <img src="https://raw.githubusercontent.com/acervenky/animated-github-badges/master/assets/pro.gif" width="50" />
  </span>
  <span style="display:flex; flex-direction:column; align-items:center; margin:10px;">
    <img src="https://raw.githubusercontent.com/acervenky/animated-github-badges/master/assets/starbadge.gif" width="50" />
  </span>
  <span style="display:flex; flex-direction:column; align-items:center; margin:10px;">
    <img src="https://raw.githubusercontent.com/acervenky/animated-github-badges/master/assets/sponsorbadge.gif" width="50" />
  </span>
</div>
<br>
<!-- devider -->
<hr style="height: 1px; border: none; background-color: #ccc; margin: 1px 0;">

<br><br>

<span style="font-size: 30px; font-weight: 900;">📊 GitHub Stats :</span>
<br><br>

[![trophy](https://github-profile-trophy.vercel.app/?username=kalimdewan97)](https://github.com/ryo-ma/github-profile-trophy)

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=kalimdewan97)](https://github.com/anuraghazra/github-readme-stats)

![GitHub stats](https://github-readme-stats.vercel.app/api?username=kalimdewan97&show_icons=true&count_private=true)

![GitHub streak stats](https://streak-stats.demolab.com/?user=kalimdewan97)

<br>
<!-- devider -->
<hr style="height: 1px; border: none; background-color: #ccc; margin: 1px 0;">
<span style="font-size: 30px; font-weight: 900;">🎖️ Additional Developer Insights :</span>
<br><br>

<p align="center">
  <img src="https://api.vaunt.dev/v1/github/entities/kalimdewan97/contributions?format=svg&private=true" height="120" />
  <br>
  <img src="https://metrics.lecoq.io/kalimdewan97" width="80%"/>
</p>
<br>
<!-- devider -->
<hr style="height: 1px; border: none; background-color: #ccc; margin: 1px 0;">
<br><br>

<h3 align="center">⭐ “Code. Create. Improve.”</h3>
<p align="center">💡 Building meaningful mobile experiences with Flutter.</p>
